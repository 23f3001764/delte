...
async def execute_query(q: str):
    try:
        ...
        if re.search(r"(\d{4}-\d{2}-\d{2}).*(\d{2}:\d{2}).*in\s+([A-Za-z0-9]+)", query, re.IGNORECASE):
            date_match = re.search(r"(\d{4}-\d{2}-\d{2})", query)
            time_match = re.search(r"(\d{2}:\d{2})", query)
            room_match = re.search(r"in\s+([A-Za-z0-9]+)", query, re.IGNORECASE)
            if date_match and time_match and room_match:
                return {"name": "schedule_meeting", "arguments": json.dumps({
                    "date": date_match.group(1),
                    "time": time_match.group(1),
                    "meeting_room": room_match.group(1).capitalize()
        })}
...
#uvicorn q8:app --host 127.0.0.1 --port 8080 --reload
#uvicorn q7:app --reload   